...
(function () {
  ...
  function parseBRLToNumber(text) {
    ...
    const only = text
      .replace(/[^\d,.-]/g, '') // tira tudo que não é número vírgula ponto ou sinal
  ...
  function formatPrecoOnInput() {
    ...
    const num = int / 100; // último dois dígitos como centavos

    precoVis.value = fmt.format(num);
    preco.value    = String(num);
  }
  ...
  form.addEventListener('submit', (e) => {
    ...
    const paradas = Array.from(
      document.querySelectorAll('#listaParadas .parada-item input[type="text"]')
    )
      .map((i) => i.value)
      .filter(Boolean);
    ...
    const rota = {
      ...
      horaIda:   d.get('partida') || '',
      horaVolta: d.get('volta') || '',
      vagas:     Number(d.get('vagas') || 0),
      veiculo:   d.get('veiculo') || '',
      dias,
      precoVis:  d.get('preco_vis') || '',
      preco:     parseBRLToNumber(d.get('preco_vis') || ''),
      paradas,
    };
