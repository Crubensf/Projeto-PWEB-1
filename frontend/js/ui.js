      ...
      el('div', { class: 'linha-info' }, [
        ...
        el('div', { class: 'horarios' }, [
          el('div', { class: 'horario' }, [
            el('span', { class: 'rotulo' }, 'Saída'),
            el('strong', { class: 'hora' }, r.horaIda || '-')
          ]),
          el('div', { class: 'horario' }, [
            el('span', { class: 'rotulo' }, 'Chegada'),
            el('strong', { class: 'hora' }, r.horaVolta || '-')
          ])
        ])
      ]),
...
export function getDiaAtivo() {
  const ativo = document.querySelector('.chip-dia.ativo');
  return ativo ? ativo.textContent.trim() : '';
}

export function setSemResultadosVisible(flag) {
  const p = document.querySelector('.sem-resultados');
  if (!p) return;
  p.hidden = !flag;
}

export function setTituloLista(texto) {
  const t = document.getElementById('tituloLista');
